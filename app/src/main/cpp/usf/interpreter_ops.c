...
void r4300i_BEQ(usf_state_t *state) {
    ...
    if (state->GPR[state->Opcode.u.b.rs].DW == state->GPR[state->Opcode.u.b.rt].DW) {
        state->JumpToLocation =
                state->PROGRAM_COUNTER + ((int16_t) state->Opcode.u.b.offset << 2) + 4;
        TestInterpreterJump(state, state->PROGRAM_COUNTER, state->JumpToLocation,
                            state->Opcode.u.b.rs, state->Opcode.u.b.rt);
    } else {
...
void r4300i_ADDI(usf_state_t *state) {
    ...
    state->GPR[state->Opcode.u.b.rt].DW = (state->GPR[state->Opcode.u.b.rs].W[0] +
                                           ((int16_t) state->Opcode.u.c.immediate));
}
...
void r4300i_SLTIU(usf_state_t *state) {
    ...
    imm64 = imm32;
    state->GPR[state->Opcode.u.b.rt].DW =
            state->GPR[state->Opcode.u.b.rs].UDW < (uint64_t) imm64 ? 1 : 0;
}

void r4300i_ANDI(usf_state_t *state) {
    state->GPR[state->Opcode.u.b.rt].DW =
            state->GPR[state->Opcode.u.b.rs].DW & state->Opcode.u.c.immediate;
}
...
void r4300i_DADDIU(usf_state_t *state) {
    state->GPR[state->Opcode.u.b.rt].DW =
            state->GPR[state->Opcode.u.b.rs].DW + (int64_t) ((int16_t) state->Opcode.u.c.immediate);
}

uint64_t LDL_MASK[8] = {0ULL, 0xFFULL, 0xFFFFULL, 0xFFFFFFULL, 0xFFFFFFFFULL, 0xFFFFFFFFFFULL,
...
void r4300i_LB(usf_state_t *state) {
    uint32_t Address =
            state->GPR[state->Opcode.u.c.base].UW[0] + (int16_t) state->Opcode.u.b.offset;
    if (state->Opcode.u.b.rt == 0) { return; }
...
void r4300i_LWL(usf_state_t *state) {
    ...
    state->GPR[state->Opcode.u.b.rt].DW = (int32_t) (state->GPR[state->Opcode.u.b.rt].W[0] &
                                                     LWL_MASK[Offset]);
...
void r4300i_LWC1(usf_state_t *state) {
    uint32_t Address = state->GPR[state->Opcode.u.c.base].UW[0] +
                       (uint32_t) ((int16_t) state->Opcode.u.b.offset);
    TEST_COP1_USABLE_EXCEPTION
    if ((Address & 3) != 0) { ADDRESS_ERROR_EXCEPTION(Address, 1); }
    if (!r4300i_LW_VAddr(state, Address,
                         &*(uint32_t *) state->FPRFloatLocation[state->Opcode.u.f.ft])) {
        //if (ShowTLBMisses) {
        // Too spammy
...
void r4300i_LDC1(usf_state_t *state) {
    ...
    TEST_COP1_USABLE_EXCEPTION
    if ((Address & 7) != 0) { ADDRESS_ERROR_EXCEPTION(Address, 1); }
    if (!r4300i_LD_VAddr(state, Address,
                         &*(uint64_t *) state->FPRDoubleLocation[state->Opcode.u.f.ft])) {
    }
}
...
void r4300i_SWC1(usf_state_t *state) {
    ...
    if (!r4300i_SW_VAddr(state, Address,
                         *(uint32_t *) state->FPRFloatLocation[state->Opcode.u.f.ft])) {
    }
}
...
void r4300i_SDC1(usf_state_t *state) {
    ...
    if (!r4300i_SD_VAddr(state, Address,
                         *(int64_t *) state->FPRDoubleLocation[state->Opcode.u.f.ft])) {
    }
}
...
void r4300i_SPECIAL_SLL(usf_state_t *state) {
    state->GPR[state->Opcode.u.e.rd].DW = (state->GPR[state->Opcode.u.b.rt].W[0]
            << state->Opcode.u.e.sa);
}

void r4300i_SPECIAL_SRL(usf_state_t *state) {
    state->GPR[state->Opcode.u.e.rd].DW = (int32_t) (state->GPR[state->Opcode.u.b.rt].UW[0]
            >> state->Opcode.u.e.sa);
}
...
void r4300i_SPECIAL_SLLV(usf_state_t *state) {
    ...
    state->GPR[state->Opcode.u.e.rd].DW = (state->GPR[state->Opcode.u.b.rt].W[0]
            << (state->GPR[state->Opcode.u.b.rs].UW[0] & 0x1F));
}
...
void r4300i_SPECIAL_DSLLV(usf_state_t *state) {
    state->GPR[state->Opcode.u.e.rd].DW =
            state->GPR[state->Opcode.u.b.rt].DW << (state->GPR[state->Opcode.u.b.rs].UW[0] & 0x3F);
}

void r4300i_SPECIAL_DSRLV(usf_state_t *state) {
    state->GPR[state->Opcode.u.e.rd].UDW =
            state->GPR[state->Opcode.u.b.rt].UDW >> (state->GPR[state->Opcode.u.b.rs].UW[0] & 0x3F);
}
...
void r4300i_SPECIAL_MULT(usf_state_t *state) {
    state->HI.DW = (int64_t) (state->GPR[state->Opcode.u.b.rs].W[0]) *
                   (int64_t) (state->GPR[state->Opcode.u.b.rt].W[0]);
    state->LO.DW = state->HI.W[0];
    state->HI.DW = state->HI.W[1];
}

void r4300i_SPECIAL_MULTU(usf_state_t *state) {
    state->HI.DW = (uint64_t) (state->GPR[state->Opcode.u.b.rs].UW[0]) *
                   (uint64_t) (state->GPR[state->Opcode.u.b.rt].UW[0]);
    state->LO.DW = state->HI.W[0];
    state->HI.DW = state->HI.W[1];
}
...
void r4300i_SPECIAL_DIV(usf_state_t *state) {
    if (state->GPR[state->Opcode.u.b.rt].UDW != 0) {
        state->LO.DW =
                state->GPR[state->Opcode.u.b.rs].W[0] / state->GPR[state->Opcode.u.b.rt].W[0];
        state->HI.DW =
                state->GPR[state->Opcode.u.b.rs].W[0] % state->GPR[state->Opcode.u.b.rt].W[0];
    } else {
    }
...
void r4300i_SPECIAL_DIVU(usf_state_t *state) {
    if (state->GPR[state->Opcode.u.b.rt].UDW != 0) {
        state->LO.DW = (int32_t) (state->GPR[state->Opcode.u.b.rs].UW[0] /
                                  state->GPR[state->Opcode.u.b.rt].UW[0]);
        state->HI.DW = (int32_t) (state->GPR[state->Opcode.u.b.rs].UW[0] %
                                  state->GPR[state->Opcode.u.b.rt].UW[0]);
    } else {
    }
}
...
void r4300i_SPECIAL_DMULT(usf_state_t *state) {
    ...
    state->LO.UDW = (uint64_t) state->GPR[state->Opcode.u.b.rs].UW[0] *
                    (uint64_t) state->GPR[state->Opcode.u.b.rt].UW[0];
    Tmp[0].UDW = (int64_t) state->GPR[state->Opcode.u.b.rs].W[1] *
                 (int64_t) (uint64_t) state->GPR[state->Opcode.u.b.rt].UW[0];
    Tmp[1].UDW = (int64_t) (uint64_t) state->GPR[state->Opcode.u.b.rs].UW[0] *
                 (int64_t) state->GPR[state->Opcode.u.b.rt].W[1];
    state->HI.UDW = (int64_t) state->GPR[state->Opcode.u.b.rs].W[1] *
                    (int64_t) state->GPR[state->Opcode.u.b.rt].W[1];
...
void r4300i_SPECIAL_DMULTU(usf_state_t *state) {
    ...
    state->LO.UDW = (uint64_t) state->GPR[state->Opcode.u.b.rs].UW[0] *
                    (uint64_t) state->GPR[state->Opcode.u.b.rt].UW[0];
    Tmp[0].UDW = (uint64_t) state->GPR[state->Opcode.u.b.rs].UW[1] *
                 (uint64_t) state->GPR[state->Opcode.u.b.rt].UW[0];
    Tmp[1].UDW = (uint64_t) state->GPR[state->Opcode.u.b.rs].UW[0] *
...
void r4300i_SPECIAL_AND(usf_state_t *state) {
    state->GPR[state->Opcode.u.e.rd].DW =
            state->GPR[state->Opcode.u.b.rs].DW & state->GPR[state->Opcode.u.b.rt].DW;
}
...
void r4300i_SPECIAL_NOR(usf_state_t *state) {
    state->GPR[state->Opcode.u.e.rd].DW = ~(state->GPR[state->Opcode.u.b.rs].DW |
                                            state->GPR[state->Opcode.u.b.rt].DW);
}
...
void r4300i_SPECIAL_DSLL(usf_state_t *state) {
    state->GPR[state->Opcode.u.e.rd].DW = (state->GPR[state->Opcode.u.b.rt].DW
            << state->Opcode.u.e.sa);
}

void r4300i_SPECIAL_DSRL(usf_state_t *state) {
    state->GPR[state->Opcode.u.e.rd].UDW = (state->GPR[state->Opcode.u.b.rt].UDW
            >> state->Opcode.u.e.sa);
}
...
void r4300i_COP1_S_ADD(usf_state_t *state) {
    TEST_COP1_USABLE_EXCEPTION
    *(float *) state->FPRFloatLocation[state->Opcode.u.f.fd] = (
            *(float *) state->FPRFloatLocation[state->Opcode.u.f.fs] +
            *(float *) state->FPRFloatLocation[state->Opcode.u.f.ft]);
}
...
void r4300i_COP1_S_SQRT(usf_state_t *state) {
    TEST_COP1_USABLE_EXCEPTION
    *(float *) state->FPRFloatLocation[state->Opcode.u.f.fd] = (float) sqrt(
            *(float *) state->FPRFloatLocation[state->Opcode.u.f.fs]);
}

void r4300i_COP1_S_ABS(usf_state_t *state) {
    TEST_COP1_USABLE_EXCEPTION
    *(float *) state->FPRFloatLocation[state->Opcode.u.f.fd] = (float) fabs(
            *(float *) state->FPRFloatLocation[state->Opcode.u.f.fs]);
}
...
void r4300i_COP1_S_NEG(usf_state_t *state) {
    TEST_COP1_USABLE_EXCEPTION
    *(float *) state->FPRFloatLocation[state->Opcode.u.f.fd] = (
            *(float *) state->FPRFloatLocation[state->Opcode.u.f.fs] * -1.0f);
}

void r4300i_COP1_S_TRUNC_L(usf_state_t *state) {
    TEST_COP1_USABLE_EXCEPTION
    //_controlfp(_RC_CHOP,_MCW_RC);
    Float_RoundToInteger64(&*(int64_t *) state->FPRDoubleLocation[state->Opcode.u.f.fd],
                           &*(float *) state->FPRFloatLocation[state->Opcode.u.f.fs]);
}
...
void r4300i_COP1_S_ROUND_W(usf_state_t *state) {
    TEST_COP1_USABLE_EXCEPTION
    Float_RoundToInteger32(&*(int32_t *) state->FPRFloatLocation[state->Opcode.u.f.fd],
                           &*(float *) state->FPRFloatLocation[state->Opcode.u.f.fs]);
}
...
void r4300i_COP1_D_ADD(usf_state_t *state) {
    TEST_COP1_USABLE_EXCEPTION
    *(double *) state->FPRDoubleLocation[state->Opcode.u.f.fd] =
            *(double *) state->FPRDoubleLocation[state->Opcode.u.f.fs] +
            *(double *) state->FPRDoubleLocation[state->Opcode.u.f.ft];
}
...
void r4300i_COP1_D_SQRT(usf_state_t *state) {
    TEST_COP1_USABLE_EXCEPTION
    *(double *) state->FPRDoubleLocation[state->Opcode.u.f.fd] = (double) sqrt(
            *(double *) state->FPRDoubleLocation[state->Opcode.u.f.fs]);
}

void r4300i_COP1_D_ABS(usf_state_t *state) {
    TEST_COP1_USABLE_EXCEPTION
    *(double *) state->FPRDoubleLocation[state->Opcode.u.f.fd] = fabs(
            *(double *) state->FPRDoubleLocation[state->Opcode.u.f.fs]);
}
...
void r4300i_COP1_D_NEG(usf_state_t *state) {
    TEST_COP1_USABLE_EXCEPTION
    *(double *) state->FPRDoubleLocation[state->Opcode.u.f.fd] = (
            *(double *) state->FPRDoubleLocation[state->Opcode.u.f.fs] * -1.0);
}

void r4300i_COP1_D_TRUNC_L(usf_state_t *state) {    //added by Witten
    TEST_COP1_USABLE_EXCEPTION
    Double_RoundToInteger64(&*(int64_t *) state->FPRFloatLocation[state->Opcode.u.f.fd],
                            &*(double *) state->FPRDoubleLocation[state->Opcode.u.f.fs]);
}
...
void r4300i_COP1_D_FLOOR_L(usf_state_t *state) {    //added by Witten
    TEST_COP1_USABLE_EXCEPTION
    Double_RoundToInteger64(&*(int64_t *) state->FPRDoubleLocation[state->Opcode.u.f.fd],
                            &*(double *) state->FPRFloatLocation[state->Opcode.u.f.fs]);
}

void r4300i_COP1_D_ROUND_W(usf_state_t *state) {
    TEST_COP1_USABLE_EXCEPTION
    Double_RoundToInteger32(&*(int32_t *) state->FPRFloatLocation[state->Opcode.u.f.fd],
                            &*(double *) state->FPRDoubleLocation[state->Opcode.u.f.fs]);
}
...
void r4300i_COP1_D_FLOOR_W(usf_state_t *state) {    //added by Witten
    TEST_COP1_USABLE_EXCEPTION
    Double_RoundToInteger32(&*(int32_t *) state->FPRDoubleLocation[state->Opcode.u.f.fd],
                            &*(double *) state->FPRFloatLocation[state->Opcode.u.f.fs]);
}
