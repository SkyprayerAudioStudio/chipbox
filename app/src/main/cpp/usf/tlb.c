...
void SetupTLB_Entry(usf_state_t *state, int Entry) {
    ...
    state->FastTlb[FastIndx].VEND =
            state->FastTlb[FastIndx].VSTART + (state->tlb[Entry].PageMask.b.Mask << 12) + 0xFFF;
    ...
    state->FastTlb[FastIndx].GLOBAL =
            state->tlb[Entry].EntryLo0.b.GLOBAL & state->tlb[Entry].EntryLo1.b.GLOBAL;
    state->FastTlb[FastIndx].ValidEntry = 0;

    FastIndx = (Entry << 1) + 1;
    state->FastTlb[FastIndx].VSTART = (state->tlb[Entry].EntryHi.b.VPN2 << 13) +
                                      ((state->tlb[Entry].PageMask.b.Mask << 12) + 0xFFF + 1);
    state->FastTlb[FastIndx].VEND =
            state->FastTlb[FastIndx].VSTART + (state->tlb[Entry].PageMask.b.Mask << 12) + 0xFFF;
    ...
    for (FastIndx = Entry << 1; FastIndx <= (Entry << 1) + 1; FastIndx++) {
        ...
        if (state->FastTlb[FastIndx].VSTART >= 0x80000000 &&
            state->FastTlb[FastIndx].VEND <= 0xBFFFFFFF) {
            continue;
        }
        ...
        //test if overlap
        state->FastTlb[FastIndx].ValidEntry = 1;
        for (count = state->FastTlb[FastIndx].VSTART;
             count < state->FastTlb[FastIndx].VEND; count += 0x1000) {
            state->TLB_Map[count >> 12] = ((uintptr_t) state->N64MEM +
                                           (count - state->FastTlb[FastIndx].VSTART +
                                            state->FastTlb[FastIndx].PHYSSTART)) - count;
        }
    }
}
...
void TLB_Probe(usf_state_t *state) {
    ...
    for (Counter = 0; Counter < 32; Counter++) {
        uint32_t TlbValue =
                state->tlb[Counter].EntryHi.Value & (~state->tlb[Counter].PageMask.b.Mask << 13);
        ...
        if (TlbValue == EntryHi) {
            ...
            uint32_t SameAsid = ((state->tlb[Counter].EntryHi.Value & 0xFF) ==
...
uint32_t TranslateVaddr(usf_state_t *state, uintptr_t *Addr) {
    ...
    *Addr = (uintptr_t) (
            (uint8_t *) (state->TLB_Map[((*Addr) & 0xffffffff) >> 12] + ((*Addr) & 0xffffffff)) -
            (uintptr_t) state->N64MEM);
    return 1;
}
