...
//_asm int 3
    ...
    state->GPR[4].UW[0] = A1;
    state->GPR[5].UW[0] = NEXT + 0xC;
    state->GPR[6].UW[0] = 0x10;
    alCopy(state, 0);
        ...
        if (nodeNext != 0) {
            ...
            if (DeltaTime < nextDelta) {
                ...
                *(uint32_t *) PageVRAM(nextItem + 8) = nextItemDelta - DeltaTime;
                ...
                state->GPR[4].UW[0] = item;
                state->GPR[5].UW[0] = node;
                alLink(state, 0);
                return 1;
            } else {
                node = nodeNext;
...
int alAudioFrame(usf_state_t *state, int paddr) {
    ...
    for (*paramSamples = __nextSampleTime(state, driver, &client);
         (*paramSamples - *curSamples) < outLen; *paramSamples = __nextSampleTime(state, driver,
                                                                                  &client)) {
