...
uint8_t *PageROM(usf_state_t *state, uint32_t addr) {
    return (state->ROMPages[addr / 0x10000]) ? state->ROMPages[addr / 0x10000] + (addr % 0x10000)
                                             : &state->EmptySpace;
}
...
int32_t Allocate_Memory(void *state) {
    ...
    USF_STATE->MemChunk = (uint8_t *) large_alloc(
            0x100000 * sizeof(uintptr_t) + 0x1D000 + USF_STATE->RdramSize);
    ...
    USF_STATE->Registers = (N64_REGISTERS *) ((uintptr_t) USF_STATE->MemChunk +
                                              0x100000 * sizeof(uintptr_t));
...
void Release_Memory(usf_state_t *state) {
    ...
    if (state->MemChunk != 0) { large_free(state->MemChunk, 0x100000 * sizeof(uintptr_t) + 0x1D000 +
                                                            state->RdramSize);
        state->MemChunk = 0;
    }
...
int32_t
r4300i_LB_NonMemory(usf_state_t *state, uint32_t PAddr, uint32_t *Value, uint32_t SignExtend) {
...
int32_t
r4300i_LH_NonMemory(usf_state_t *state, uint32_t PAddr, uint32_t *Value, int32_t SignExtend) {
    (void) state;
    (void) SignExtend;
    switch (PAddr & 0xFFF00000) {
        default:
            ...
            switch (PAddr) {
                ...
                case 0x04040010:
                    ...
                    if ((Value & SP_CLR_INTR_BREAK) !=
                        0) { SP_STATUS_REG &= ~SP_STATUS_INTR_BREAK; }
            ...
            switch (PAddr) {
                ...
                case 0x0410000C:
                    if ((Value & DPC_CLR_XBUS_DMEM_DMA) !=
                        0) { DPC_STATUS_REG &= ~DPC_STATUS_XBUS_DMEM_DMA; }
