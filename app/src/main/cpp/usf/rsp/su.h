...
static void MFC0(usf_state_t *state, int rt, int rd) {
    ...
    if (rd == 0x7) /* SP_SEMAPHORE_REG */
    {
        if (CFG_MEND_SEMAPHORE_LOCK == 0)
            return;
        SP_SEMAPHORE_REG = 0x00000001;
        SP_STATUS_REG |= 0x00000001; /* temporary bit to break CPU */
        return;
    }
    if (rd == 0x4) /* SP_STATUS_REG */
    {
        ...
        if (state->MFC0_count[rt] > 07)
            SP_STATUS_REG |= 0x00000001; /* Let OS restart the task. */
#endif
    }
    return;
}
...
static void MT_SP_STATUS(usf_state_t *state, int rt) {
    ...
    MI_INTR_REG &= ~((state->SR[rt] & 0x00000008) >> 3); /* SP_CLR_INTR */
    MI_INTR_REG |= ((state->SR[rt] & 0x00000010) >> 4); /* SP_SET_INTR */
    SP_STATUS_REG |= (state->SR[rt] & 0x00000010) >> 4; /* int set halt */
    SP_STATUS_REG &= ~(!!(state->SR[rt] & 0x00000020) << 5);
    /* SP_STATUS_REG |=  (!!(state->SR[rt] & 0x00000040) <<  5); */
    SP_STATUS_REG &= ~(!!(state->SR[rt] & 0x00000080) << 6);
    SP_STATUS_REG |= (!!(state->SR[rt] & 0x00000100) << 6);
    SP_STATUS_REG &= ~(!!(state->SR[rt] & 0x00000200) << 7);
    SP_STATUS_REG |= (!!(state->SR[rt] & 0x00000400) << 7);
    SP_STATUS_REG &= ~(!!(state->SR[rt] & 0x00000800) << 8);
    SP_STATUS_REG |= (!!(state->SR[rt] & 0x00001000) << 8);
    SP_STATUS_REG &= ~(!!(state->SR[rt] & 0x00002000) << 9);
    SP_STATUS_REG |= (!!(state->SR[rt] & 0x00004000) << 9);
    SP_STATUS_REG &= ~(!!(state->SR[rt] & 0x00008000) << 10);
    SP_STATUS_REG |= (!!(state->SR[rt] & 0x00010000) << 10);
    SP_STATUS_REG &= ~(!!(state->SR[rt] & 0x00020000) << 11);
    SP_STATUS_REG |= (!!(state->SR[rt] & 0x00040000) << 11);
    SP_STATUS_REG &= ~(!!(state->SR[rt] & 0x00080000) << 12);
    SP_STATUS_REG |= (!!(state->SR[rt] & 0x00100000) << 12);
    SP_STATUS_REG &= ~(!!(state->SR[rt] & 0x00200000) << 13);
    SP_STATUS_REG |= (!!(state->SR[rt] & 0x00400000) << 13);
    SP_STATUS_REG &= ~(!!(state->SR[rt] & 0x00800000) << 14);
    SP_STATUS_REG |= (!!(state->SR[rt] & 0x01000000) << 14);
    return;
}
...
static void MT_CMD_STATUS(usf_state_t *state, int rt) {
    ...
    DPC_STATUS_REG |= (!!(state->SR[rt] & 0x00000008) << 1);
    DPC_STATUS_REG &= ~(!!(state->SR[rt] & 0x00000010) << 2);
    DPC_STATUS_REG |= (!!(state->SR[rt] & 0x00000020) << 2);
/* Some NUS-CIC-6105 SP tasks try to clear some zeroed DPC registers. */
    DPC_TMEM_REG &= !(state->SR[rt] & 0x00000040) * -1;
    /* DPC_PIPEBUSY_REG &= !(state->SR[rt] & 0x00000080) * -1; */
    /* DPC_BUFBUSY_REG  &= !(state->SR[rt] & 0x00000100) * -1; */
    DPC_CLOCK_REG &= !(state->SR[rt] & 0x00000200) * -1;
    return;
}
...
void SP_DMA_READ(usf_state_t * state)
{
    ...
    length = (SP_RD_LEN_REG & 0x00000FFF) >>  0;
    count  = (SP_RD_LEN_REG & 0x000FF000) >> 12;
    skip   = (SP_RD_LEN_REG & 0xFFF00000) >> 20;
    ...
    { /* `count` always starts > 0, so we begin with `do` instead of `while`. */
        ...
        do
        {
            offC = (count*length + SP_MEM_ADDR_REG + i) & 0x00001FF8;
            offD = (count*skip + SP_DRAM_ADDR_REG + i) & 0x00FFFFF8;
            memcpy(state->DMEM + offC, state->RDRAM + offD, 8);
            i += 0x008;
        } while (i < length);
    } while (count);
    SP_DMA_BUSY_REG = 0x00000000;
    SP_STATUS_REG &= ~0x00000004; /* SP_STATUS_DMABUSY */
}
void SP_DMA_WRITE(usf_state_t * state)
{
    ...
    length = (SP_WR_LEN_REG & 0x00000FFF) >>  0;
    count  = (SP_WR_LEN_REG & 0x000FF000) >> 12;
    skip   = (SP_WR_LEN_REG & 0xFFF00000) >> 20;
    /* length |= 07; // already corrected by mtc0 */
    ++length;
    ++count;
    skip += length;
    ...
    { /* `count` always starts > 0, so we begin with `do` instead of `while`. */
        ...
        do
        {
            offC = (count*length + SP_MEM_ADDR_REG + i) & 0x00001FF8;
            offD = (count*skip + SP_DRAM_ADDR_REG + i) & 0x00FFFFF8;
            memcpy(state->RDRAM + offD, state->DMEM + offC, 8);
            i += 0x000008;
        } while (i < length);
    } while (count);
    SP_DMA_BUSY_REG = 0x00000000;
...
rwR_VCE(usf_state_t *state) { /* never saw a game try to read VCE out to a scalar GPR yet */
...
static void rwW_VCE(usf_state_t *state,
                    unsigned short VCE) { /* never saw a game try to write VCE using a scalar GPR yet */
    register int i;
...
INLINE static void SDV(usf_state_t *state, int vt, int element, int offset, int base) {
    ...
    addr = (state->SR[base] + 8 * offset) & 0x00000FFF;
    if (e > 0x8 || (e & 0x1)) { /* Illegal elements with Boss Game Studios publications. */
        register int i;
...
INLINE static void LUV(usf_state_t *state, int vt, int element, int offset, int base) {
    ...
    addr = (state->SR[base] + 8 * offset) & 0x00000FFF;
    if (e != 0x0) { /* "Mia Hamm Soccer 64" SP exception override (zilmar) */
        addr += -e & 0xF;
        for (b = 0; b < 8; b++) {
...
INLINE static void SQV(usf_state_t *state, int vt, int element, int offset, int base) {
    ...
    addr = (state->SR[base] + 16 * offset) & 0x00000FFF;
    if (e != 0x0) { /* happens with "Mia Hamm Soccer 64" */
        register int i;
...
void ULW(usf_state_t *state, int rd, uint32_t addr) { /* "Unaligned Load Word" */
    union {
        unsigned char B[4];
        signed char SB[4];
        unsigned short H[2];
...
void USW(usf_state_t *state, int rs, uint32_t addr) { /* "Unaligned Store Word" */
    union {
        unsigned char B[4];
        signed char SB[4];
        unsigned short H[2];
