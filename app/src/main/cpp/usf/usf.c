    ...
    if (temp == 0x34365253) { //there is a rom section
        ...
        while (len) {
            ...
            while (len) {
                ...
                uint32_t readLen = (((start + len) >> 16) > page) ? (((page + 1) << 16) - start)
                                                                  : len;
...
const char *usf_render(void *state, int16_t *buffer, size_t count, int32_t *sample_rate) {
    ...
    if (USF_STATE->samples_in_buffer) {
        ...
        if (USF_STATE->samples_in_buffer) {
            memmove(USF_STATE->samplebuf, USF_STATE->samplebuf + do_max * 2,
                    sizeof(int16_t) * 2 * USF_STATE->samples_in_buffer);
            return 0;
        }
