...
int32_t DelaySlotEffectsJump(usf_state_t *state, uint32_t JumpPC) {
    ...
    switch (Command.u.b.op) {
        case R4300i_SPECIAL:
            switch (Command.u.e.funct) {
                ...
                case R4300i_SPECIAL_JALR:
                    return DelaySlotEffectsCompare(state, JumpPC, Command.u.b.rs, 31);
            }
            break;
        case R4300i_REGIMM:
        ...
        case R4300i_JAL:
        case R4300i_SPECIAL_JALR:
            return DelaySlotEffectsCompare(state, JumpPC, 31, 0);
            break;
        case R4300i_J:
            return 0;
        case R4300i_BEQ:
                ...
                case R4300i_COP1_BC:
                    switch (Command.u.f.ft) {
                        ...
                        case R4300i_COP1_BC_BCTL: {
                            ...
                            if (!r4300i_LW_VAddr(state, JumpPC + 4,
                                                 &NewCommand.u.Hex)) { return 1; }

                            EffectDelaySlot = 0;
                            if (NewCommand.u.b.op == R4300i_CP1) {
                                if (NewCommand.u.f.fmt == R4300i_COP1_S &&
                                    (NewCommand.u.e.funct & 0x30) == 0x30) {
                                    EffectDelaySlot = 1;
                                }
                                if (NewCommand.u.f.fmt == R4300i_COP1_D &&
                                    (NewCommand.u.e.funct & 0x30) == 0x30) {
                                    EffectDelaySlot = 1;
                                }
                            }
...
void RefreshScreen(usf_state_t *state) {
    ...
    ChangeTimer(state, ViTimer,
                state->Timers->Timer + state->Timers->NextTimer[ViTimer] + state->VI_INTR_TIME);
